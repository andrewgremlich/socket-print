...
export class TestCylinder extends AppObject {
	...
	#radius = 75 / 2;
	#height = 50;
	$liveTestCylinderID: Subscription;
	$liveTestCylinderHeight: Subscription;

	private constructor() {
		super();

		this.$liveTestCylinderID = liveQuery(() =>
			getTestCylinderInnerDiameter(),
		).subscribe((diameter) => {
	...
	dispose() {
		if (this.$liveTestCylinderID) {
			this.$liveTestCylinderID.unsubscribe();
			this.$liveTestCylinderID = null;
		}
